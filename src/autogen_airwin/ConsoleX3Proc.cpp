...
void ConsoleX3::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
    float* in1  =  inputs[0];
    float* in2  =  inputs[1];
    ...
    while (--sampleFrames >= 0)
    {
		...
		if (bezier[bezier_cycle] > bezierTrim) {bezier[bezier_cycle] = 0.0;
			...
			bezier[bezierHigh_A] = (pow(HIG+0.5,3.0)+3.0)*0.25;
			bezier[bezierHmid_A] = (pow(HMG+0.5,3.0)+3.0)*0.25;
			bezier[bezierLmid_A] = (pow(LMG+0.5,3.0)+3.0)*0.25;
			bezier[bezierBass_A] = (pow(BSG+0.5,3.0)+3.0)*0.25;
...
void ConsoleX3::processDoubleReplacing(double **inputs, double **outputs, VstInt32 sampleFrames) 
{
    double* in1  =  inputs[0];
    double* in2  =  inputs[1];
