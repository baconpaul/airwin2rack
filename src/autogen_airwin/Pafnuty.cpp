...
bool Pafnuty::parameterTextToValue(VstInt32 index, const char *text, float &value) {
    switch(index) {
    ...
    case kParamK: { auto b = string2float(text, value); if (b) { value = (value + 1.0) / (2.0); } return b; break; }
    case kParamL: { auto b = string2float(text, value); if (b) { value = (value + 1.0) / (2.0); } return b; break; }
    case kParamM: { auto b = string2float(text, value); if (b) { value = (value + 1.0) / (2.0); } return b; break; }

    }
    return false;
}
bool Pafnuty::canConvertParameterTextToValue(VstInt32 index) {
    switch(index) {
