...
bool OneCornerClip::parameterTextToValue(VstInt32 index, const char *text, float &value) {
    switch(index) {
    case kParamA: { auto b = string2float(text, value); if (b) { value = (value + 12.0) / (36.0); } return b; break; }
    case kParamB: { auto b = string2dBNorm(text, value); return b; break; }
...
bool OneCornerClip::canConvertParameterTextToValue(VstInt32 index) {
    switch(index) {
