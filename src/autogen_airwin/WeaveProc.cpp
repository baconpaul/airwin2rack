...
void Weave::processReplacing(float **inputs, float **outputs, VstInt32 sampleFrames) 
{
    float* in1  =  inputs[0];
    float* in2  =  inputs[1];
    ...
    while (--sampleFrames >= 0)
    {
		...
		if (inputSampleR != 0.0) {
			...
			inputSampleR *= 1.27;
			long double weave = sqrt(fabs(inputSampleR));
			long double spiral = inputSampleR * weave;
			long double X = spiral; X *= X;
			long double temp = spiral * X;
			spiral -= temp*0.16666666666666666666666666666666666; temp *= X;//-8 dB
			spiral += temp*0.00833333333333333333333333333333333; temp *= X;//-20 dB
...
void Weave::processDoubleReplacing(double **inputs, double **outputs, VstInt32 sampleFrames) 
{
    double* in1  =  inputs[0];
    double* in2  =  inputs[1];
